...
exports.obtenerComprasFiltradas = async ({ rut, fechaInicio, fechaFin, pagina = 1, limite = 10, busqueda }) => {
  const offset = (pagina - 1) * limite;
  const filtros = [];
  const valores = [];

  if (busqueda) {
    filtros.push('(u.rut ILIKE $' + (valores.length + 1) + ' OR u.email ILIKE $' + (valores.length + 1) + ')');
    valores.push(`%${busqueda}%`);
  }
  ...
  const { rows } = await pool.query(query, valores);
  return { resultados: rows, totalPaginas: 1 }; // totalPaginas puede calcularse mejor si quieres
};
